...
function initializeVariables() {
    ...
    let lastDirection; //avoid loop in the same gesture
...
async function populateCameraList() {
    ...
    // Ngắt luồng video từ webcam hiện tại (nếu có)
...
////////////////
function handleChunk(frame) { //Tạo khung video
...
async function openWebSocket() {
    ...
    const selectedCameraId = cameraSelect.value; // Lấy giá trị camera được chọn

    // Ngắt luồng video từ webcam trước đó (nếu có)
    if (videoElement.srcObject) {
        const tracks = videoElement.srcObject.getTracks();
        tracks.forEach(track => track.stop());
        videoElement.srcObject = null;
    ...
    websocket.onopen = async () => {
        if (device && selectedCameraId) { // Chỉ bắt đầu luồng video nếu có thiết bị bluetooth và camera được chọn
            await getVideoStream({
                deviceId: selectedCameraId, // Truyền camera ID được chọn vào hàm getVideoStream
            }).then(async (stream) => {
...
function stop() {
    // Đóng stream video từ camera nếu đang mở
    const videoElement = document.getElementById("videoElement");
    if (videoElement.srcObject) {
        const tracks = videoElement.srcObject.getTracks();
        tracks.forEach(track => track.stop());
    ...
    // Ngắt kết nối Bluetooth nếu đang kết nối
    if (device && device.gatt?.connected) {
        device.gatt.disconnect();
    }
    
    // Ngừng video trên canvas
    const canvasElement = document.getElementById("canvasElement");
...
async function detectHandGestureFromVideo(gestureRecognizer, stream) { // hàm của javarspript
